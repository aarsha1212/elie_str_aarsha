...
from dataset.SID_Sony1 import SIDSonyTestDataset_fast_cc
...
def main():
    ...
    if torch.cuda.is_available():
        ...
        craft_net.load_state_dict(copyStateDict(torch.load(Cfg.craft_pretrained_model, map_location='cpu')))
    craft_net.to(device)
    craft_net.eval()
    ...
    with torch.no_grad():
        ...
        for sample in tqdm(test_dataloader, desc='Testing Sample:'):
            ...
            # Extract the file name without extension from the input filename
            in_fn_name = os.path.splitext(in_fn)[0]

            # Ensure that the file extension is valid
            valid_extensions = ['.png', '.jpg', '.jpeg']
            if in_fn.endswith(tuple(valid_extensions)):
                output_filename = f"{in_fn_name}_output{os.path.splitext(in_fn)[1]}"
                cv2.imwrite(os.path.join(out_img_path, output_filename), out_img)
            else:
                raise RuntimeError("Invalid file extension for output image!")
