...
import utils
from configs.cfg_SID_Sony1 import Cfg
from CRAFTpytorch.craft import CRAFT
from dataset.SID_Sony1 import SIDSonyTestDataset_fast_cc as sid_test_dataset
from dataset.SID_Sony1 import SIDSonyTrainDataset_fast_cc as sid_train_dataset
from dataset.SID_Sony1 import SIDSonyTrainDataset_fast_cc
from unet1 import GrayEdgeAttentionUNet
...
def main():
    ...
    craft_net.load_state_dict(copyStateDict(torch.load(Cfg.craft_pretrained_model, map_location=device)))
    ...
    train_dataset = sid_train_dataset(
        list_file=Cfg.train_list_file, root_dir=Cfg.dataset_dir, ps=Cfg.ps)
    train_dataloader = DataLoader(train_dataset, batch_size=Cfg.bs, shuffle=True, num_workers=8,
                                  pin_memory=True, persistent_workers=True)

    train_dataset = SIDSonyTrainDataset_fast_cc(list_file=Cfg.train_list_file, root_dir=Cfg.dataset_dir, ps=Cfg.ps)
    train_dataloader = DataLoader(train_dataset, batch_size=Cfg.bs, shuffle=True, num_workers=8,pin_memory=True, persistent_workers=True)
    ...
    for epoch in tqdm(range(lastepoch, Cfg.training_epoch), desc='Training Epoch:'):
        ...
        checkpoint_path = os.path.join(Cfg.result_dir, f'epoch_{epoch}.tar')
        utils.save_checkpoint_state(checkpoint_path, epoch, unet, optimizer, scheduler)
